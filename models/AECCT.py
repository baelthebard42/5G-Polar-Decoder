...
class SublayerConnection_original(nn.Module):
    def __init__(self, size, dropout):
        super(SublayerConnection_original, self).__init__()
        self.norm = LayerNorm(size)
        self.dropout = nn.Dropout(dropout)
...
class EncoderLayer_original(nn.Module):
    def __init__(self, size, self_attn, feed_forward, dropout):
        super(EncoderLayer_original, self).__init__()
        self.self_attn = self_attn
        self.feed_forward = feed_forward
        self.sublayer = clones(SublayerConnection_original(size, dropout), 2)
        self.size = size
...
class MultiHeadedAttention(nn.Module):
    def __init__(
        ...
        self.h = config.h
        if config.use_aap_linear_training:
            self.linears = clones(AAPLinearTraining(config.d_model, config.d_model, config=config), 4)
        elif config.use_aap_linear_inference:
            self.linears = clones(AAPLinearInference(config.d_model, config.d_model, config=config), 4)
        else:
            self.linears = clones(nn.Linear(config.d_model, config.d_model), 4)
        self.attn = None
        self.dropout = nn.Dropout(p=dropout)
        self.num_variable_nodes = config.code.n
        self.num_heads_one_ring = config.num_heads_for_one_ring
...
class PositionwiseFeedForward(nn.Module):
    def __init__(
        ...
        if config.use_aap_linear_training:
            self.w_1 = AAPLinearTraining(config.d_model, config.d_model*4, config=config)
            self.w_2 = AAPLinearTraining(config.d_model*4, config.d_model, config=config)
        elif config.use_aap_linear_inference:
            self.w_1 = AAPLinearInference(config.d_model, config.d_model*4, config=config)
            self.w_2 = AAPLinearInference(config.d_model*4, config.d_model, config=config)
        else:
            self.w_1 = nn.Linear(config.d_model, config.d_model*4)
            self.w_2 = nn.Linear(config.d_model*4, config.d_model)
        self.dropout = nn.Dropout(dropout)
...
class PositionwiseFeedForward_original(nn.Module):
    def __init__(self, d_model, d_ff, dropout=0):
        super(PositionwiseFeedForward_original, self).__init__()
...
class ECC_Transformer(nn.Module):
    def __init__(
        ...
        emb_dim = config.d_model - config.lpe_dim

        self.src_embed = torch.nn.Parameter(torch.empty(
            (code.n + code.pc_matrix.size(0), emb_dim)))
        self.decoder = Encoder(EncoderLayer(
            config.d_model, c(attn), c(ff), dropout, config), config.N_dec)
        self.oned_final_embed = torch.nn.Sequential(
            *[nn.Linear(config.d_model, 1)])
        self.out_fc = nn.Linear(code.n + code.pc_matrix.size(0), code.n)

        self.get_mask(code)
        self.create_laplacian_pe(self.get_adj_matrix(config.code).float().squeeze(0).squeeze(0))
        self.lpe_proj = nn.Linear(self.lpe.size(-1), config.lpe_dim)
        self.attn_lpe = lambda x: lpe_attn(x, config.lpe_num_heads, config.lpe_dim // config.lpe_num_heads)
...
class ECC_Transformer_original(nn.Module):
    ...
    #    print("config:",config)
        ...
        ff = PositionwiseFeedForward_original(config.d_model, config.d_model*4, dropout)

        self.src_embed = torch.nn.Parameter(torch.empty(
            (code.n + code.pc_matrix.size(0), config.d_model)))
        self.decoder = Encoder(EncoderLayer_original(
            config.d_model, c(attn), c(ff), dropout), config.N_dec)
        self.oned_final_embed = torch.nn.Sequential(
            *[nn.Linear(config.d_model, 1)])
        self.out_fc = nn.Linear(code.n + code.pc_matrix.size(0), code.n)
