...
class ECCM(torch.nn.Module):
    ...
    def forward(self, magnitude, syndrome):
        ...
        return outputs # add emb with output to stabalize
...
class ECCM_only_mamba(torch.nn.Module):
    def __init__(self, config: Config) -> None:
        super().__init__()
        self.n = config.code.n
        self.syndrom_length = config.code.pc_matrix.size(0)
    ...
    def loss(self, z_pred, z2, y):
        ...
        for i,z in enumerate(z_pred):
            if z.grad_fn is not None:
                ...
                if torch.any(torch.isnan(z)) or torch.any(z < 0) or torch.any(z > 1):
                    print("BAD z DETECTED")
                    print("min:", z.min().item(), "max:", z.max().item())
                    print("has nan:", torch.isnan(z).any().item())
                    raise RuntimeError("Invalid BCE input")
